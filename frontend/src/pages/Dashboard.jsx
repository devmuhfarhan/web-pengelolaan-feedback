...
const Dashboard = () => {
  ...
  const pieData =
    stats.status_distribution?.length > 0 ? stats.status_distribution : [];

  const pendingCount =
    pieData.find((d) => d.name === "PENDING" || d.name === "Pending")?.value ||
    0;
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Dashboard_Report_Puspadi_Bali_${new Date().toISOString().slice(0, 10)}.csv`,
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-slate-500 text-sm">
              Monitoring mission-critical feedback and program integrity.
            </p>
          </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <CardDescription className="text-xs">
                  Latest feedback submitted to the portal
                </CardDescription>
              </div>
              <Button
                variant="ghost"
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                  {stats.recent_feedbacks.slice(0, 5).map((fb) => (
                    ...
                    >
                      <td className="px-6 py-3.5 font-medium max-w-[150px] truncate text-slate-700 dark:text-slate-300">
                        {fb.program_detail?.title || "Unknown"}
                      </td>
                      ...
                      <td className="px-6 py-3.5 text-slate-400 text-xs text-right font-medium">
                        {new Date(fb.created_at).toLocaleDateString()}
                      </td>
                    </tr>
                  ))}
                ...
                <div key={index} className="mb-4 last:mb-0">
                  ...
                  <div className="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.max(5, (item.value / stats.total_feedbacks) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
  ...
  // ---- ROLE: OPERATIONAL_STAFF / FIELD_STAFF ----
  if (user?.role === "OPERATIONAL_STAFF" || user?.role === "FIELD_STAFF") {
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-slate-500 text-sm">
              {user?.role === "FIELD_STAFF"
                ? "Welcome back. Here's the latest field data."
                : "Welcome back. Here's the latest operational data."}
            </p>
          </div>
        </div>
          ...
          <Card className="bg-white dark:bg-slate-900 shadow-sm border-0 ring-1 ring-slate-200 dark:ring-slate-800">
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100">
                  {Number((stats.average_rating / 5) * 100).toFixed(1)}%
                </h3>
              </div>
          ...
          <Card className="md:col-span-2 bg-white dark:bg-slate-900 shadow-sm border-0 ring-1 ring-slate-200 dark:ring-slate-800 overflow-hidden">
            <CardHeader className="pb-4">
              <div className="flex justify-between items-center">
                <CardTitle className="text-base text-slate-800 dark:text-slate-100">
                  Regular Reporting Trend
                </CardTitle>
                <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold px-2 py-1 rounded-md">
                  Last 6 Months
                </span>
              </div>
            </CardHeader>
                  ...
                  >
                    <div className="min-w-0 pr-4">
                      <p className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">
                        {fb.program_detail?.title || "Unknown Program"}
                      </p>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[9px] font-bold tracking-wide uppercase shrink-0 ${
                        fb.status === "RESOLVED"
                          ? "bg-emerald-100/50 text-emerald-700"
                          : fb.status === "REVIEWED"
                            ? "bg-blue-100/50 text-blue-700"
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto mt-4">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 text-sm">
          Track the status of your feedback and see your impact.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="bg-white dark:bg-slate-900 shadow-sm border-0 ring-1 ring-slate-200 dark:ring-slate-800">
          <CardHeader className="text-center pb-2">
            ...
            <CardTitle className="text-3xl font-black text-slate-800 dark:text-slate-100">
              {stats.recent_feedbacks.filter((f) => f.status === "RESOLVED")
                .length || 0}
            </CardTitle>
