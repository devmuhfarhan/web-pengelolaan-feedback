                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-600 dark:text-slate-400">
                        {item.replacements && item.replacements.length > 0
                          ? [...item.replacements].sort(
                              (a, b) =>
                                new Date(b.date_replaced) -
                                new Date(a.date_replaced),
                            )[0].date_replaced
                          : item.date_replaced || "-"}
                      </span>
                    </td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1 md:col-span-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Award className="w-3.5 h-3.5" /> Program
                  </span>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
                    {detailBeneficiary.program_detail?.title || "No program"}
                  </p>
                </div>

                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <UserCheck className="w-3.5 h-3.5" /> Date Provided
                  </span>
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200">
                    {detailBeneficiary.date_provided || "-"}
                  </p>
                </div>

                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <RefreshCw className="w-3.5 h-3.5" /> Replacement History
                  </span>
                  ...
                  detailBeneficiary.replacements.length > 0 ? (
                    <ul className="text-sm font-medium text-slate-800 dark:text-slate-200 list-disc list-inside">
                      {detailBeneficiary.replacements.map((rep) => (
                        <li key={rep.id}>
                          {rep.date_replaced} ({rep.program_detail?.title})
                        </li>
                      ))}
                    </ul>
                  ) : (
